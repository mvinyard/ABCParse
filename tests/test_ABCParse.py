...
# -- import modules: -----------------------------------------------------------
from ABCParse import ABCParse, function_kwargs, as_list, get_logger
from ABCParse._logging import ABCLogger
...
class TestFunctionKwargs(unittest.TestCase):
    ...
    def test_function_kwargs_basic(self):
        """Test basic functionality of function_kwargs."""
        def test_func(a, b, c=None):
            pass
        
        kwargs = {"a": 1, "b": 2, "d": 3}
        result = function_kwargs(test_func, kwargs)
        
        self.assertEqual(result, {"a": 1, "b": 2})
        self.assertNotIn("d", result)
        
    def test_function_kwargs_with_obj(self):
        ...
        obj = TestObj()
        result = function_kwargs(test_func, obj=obj)
        
        self.assertEqual(result, {"x": 10, "y": 20})
        self.assertNotIn("z", result)
        
    def test_function_kwargs_with_ignore(self):
        """Test function_kwargs with ignore list."""
        def test_func(a, b, c):
            pass
        
        kwargs = {"a": 1, "b": 2, "c": 3}
        result = function_kwargs(test_func, kwargs, ignore=["a", "self", "kwargs"])
...
class TestAsList(unittest.TestCase):
    ...
    def test_as_list_list_value(self):
        """Test as_list with a list value."""
        result = as_list([1, 2, 3])
        self.assertEqual(result, [1, 2, 3])
        
    def test_as_list_with_target_type(self):
        """Test as_list with target type checking."""
        # Should pass
        result = as_list([1, 2, 3], target_type=int)
        self.assertEqual(result, [1, 2, 3])
        
        # Should pass with single value
        result = as_list(1, target_type=int)
        self.assertEqual(result, [1])
        
        # Should pass with multiple target types
        result = as_list([1, "string"], target_type=[int, str])
        self.assertEqual(result, [1, "string"])
        
        # Should fail
        with self.assertRaises(AssertionError):
            as_list([1, "string"], target_type=int)
...
class TestLogging(unittest.TestCase):
    ...
    def test_logger_level(self):
        ...
        logger = get_logger("test_level_logger", level="debug")
    ...
    def test_file_logging(self):
        ...
        try:
            logger = get_logger("file_logger", file_path=temp_path)
